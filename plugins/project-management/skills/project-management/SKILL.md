...
> releases, search, and actions, use the **github-cli:github-cli** skill instead.
...
See the **dev-workflow:dev-workflow** skill for planning session workflows.
...
See the **dev-workflow:dev-workflow** release reference for tagging workflows.
...
- The `gh issue create` CLI does not support a `--type` flag — assign types via GraphQL
  after creation (see github-cli:github-cli issue-type reference)
...
6. **Scope boundary**: Use this skill for `gh project *` commands. For issue/PR lifecycle (create, edit, close, label, assign), use the **github-cli:github-cli** skill
