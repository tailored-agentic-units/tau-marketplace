...
- Load skills: **github-cli:github-cli**
- Collect session metadata: issue number (required)
...
4. Verify the issue has a type assigned (the `gh` CLI does not expose issue types natively —
   use the GraphQL query from the github-cli:github-cli issue-type reference if needed)
...
6. Load domain-specific skills based on issue labels (e.g., go-patterns:go-patterns for Go development)
...
- Run through the project's validation checklist (vet, lint, tidy, etc.)
...
Then create the PR using the github-cli:github-cli skill:
